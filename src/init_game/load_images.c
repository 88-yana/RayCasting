...
/*   Updated: 2023/04/23 13:20:56 by tmuramat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*xpm_file_to_image(t_game *game, t_image *image, char *img_file)
{
	...
	img = mlx_xpm_file_to_image(game->mlx,
			img_file, &image->width, &image->height);
	if (img == NULL)
...
void	load_images(t_game *game)
{
	...
	images->north.addr = mlx_get_data_addr(north,
			&images->north.bpp, &images->north.len, &images->north.endian);
	images->south.addr = mlx_get_data_addr(south,
			&images->south.bpp, &images->south.len, &images->south.endian);
	images->east.addr = mlx_get_data_addr(east,
			&images->east.bpp, &images->east.len, &images->east.endian);
	images->west.addr = mlx_get_data_addr(west,
			&images->west.bpp, &images->west.len, &images->west.endian);
}
